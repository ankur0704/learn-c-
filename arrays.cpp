...
void sortZeroOne(int arr[], int n){
    ...
    for(int i=0; i<n; i++){
        ...
        //step2 place all zero first
        int i;
        for(int i=0; i<zeroCount; i++){
            arr[i]=0;
        }
        // step3 place all ones first
        for(int j=i; j<n; j++){
            arr[j] = 1;
        }
    }
...
//shirt arrays elements right by 1(right shift)
...
void shiftArray(int arr[], int n){
    ...
    //step2 copy temp into 0th index
    arr[0] = temp;
};
...
int main(){
    ...
    shiftArray(arr, n);          //created function shiftArray

    for(int i=0; i<n; i++){
        cout<<arr[i]<<" ";               //printed shiftArray
    }
};
